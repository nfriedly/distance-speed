...
const handleCommand = (drivers, line) => {
  if (line.command === CMD_DRIVER) {
    drivers[line.driver] = { highwayMiles: 0, sideStreetMiles: 0, hours: 0 };
    ...
    if (isSpeedInRange(miles, hours)) {
      const mph = miles / hours;
      if (mph >= 50) {
        drivers[line.driver].highwayMiles += miles;
      } else {
        drivers[line.driver].sideStreetMiles += miles;
      }
...
const analyze = drivers =>
  Object.entries(drivers)
    .map(([driver, { highwayMiles, sideStreetMiles, hours }]) => {
      const miles = highwayMiles + sideStreetMiles;
      return {
        driver,
        miles,
        mph: hours > 0 ? miles / hours : undefined,
        percentHighway: hours > 0 ? (highwayMiles / miles) * 100 : undefined
      };
    })
